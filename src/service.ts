...
class MysqlErrorHandle{
    ...
    verificaErroDB(){
        ...
        if (this.error instanceof Error && 'code' in this.error && (this.error.code === 'ECONNREFUSED' || this.error.code === 'EAI_AGAIN')) {
            this.res.status(500).json({ mensagem: "ERRO: LIGUE O LARAGON e confira o usuário e senha da conexão" })
        } else if (this.error instanceof Error && 'code' in this.error && this.error.code === 'ENOTFOUND') {
            this.res.status(500).json({ mensagem: "ERRO: Você digitou algo errado no host da conexão" })
        } else if (this.error instanceof Error && 'code' in this.error && this.error.code === 'ER_BAD_DB_ERROR') {
        ...
        } else if (this.error instanceof Error && 'code' in this.error && this.error.code === 'ER_ACCESS_DENIED_ERROR') {
            this.res.status(500).json({ mensagem: "ERRO: Confira usuario e senha na conexão" })
        } else if (this.error instanceof Error && 'code' in this.error && this.error.code === 'ER_PARSE_ERROR') {
